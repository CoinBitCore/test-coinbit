Coinbit Core version 0.13.0 is now available from:

  <https://coinbit.org/bin/coinbit-core-0.13.0/>
...
- Adding `dbcache=100` in coinbit.conf
...
For example:
    ...
    $ src/coinbit-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Coinbit Core code base has
...
HD wallets are incompatible with older versions of Coinbit Core.

[Pull request](https://github.com/CoinBitCore/coinbit/pull/8035/files), [BIP 32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki)
...
proposed parameters, a future release of Coinbit Core is expected that
...
The two phases are distinct in the Coinbit-Qt GUI. During the first one,
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/CoinBitCore/coinbit/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for coinbit-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use CTB/kB (MarcoFalke)
...
- #7692 `29b2be6` Remove P2P alert system (CTBdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (CTBdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (CTBdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (MarcoFalke)
- #7658 `c15eb28` Add curl to Gitian setup instructions (CTBdrak)
...
- #7613 `3798e5d` Add autocomplete to coinbit-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add coinbit address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: coinbit-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from coinbitd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Coinbit-Core repository and website to the About Dialog (MarcoFalke)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (CTBdrak)
...
- #7773 `7c80e72` Fix comments in tests (CTBdrak)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for coinbitd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Coinbit-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of coinbitd startup (laanwj)
...
- #7975 `fc23fee` Update coinbit-core GitHub links (MarcoFalke)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (CTBdrak)
