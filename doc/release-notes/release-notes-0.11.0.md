Coinbit Core version 0.11.0 is now available from:

  <https://coinbit.org/bin/coinbit-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer CTB/kB fee to be
...
For example, add the following to `coinbit.conf`:
...
related to the blockchain in the coinbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Coinbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Coinbit Core will 
...
For an example script see [coinbit-submittx](https://github.com/laanwj/coinbit-submittx).
...
- #5334 `cf87536` libcoinbitconsensus: Add pkg-config support
...
- #5729 `9c4a5a5` Allow unit changes for read-only CoinbitAmountField
- #5753 `0f44672` Add coinbit logo to about screen
...
- #5809 `46bfbe7` Add coinbit-cli man page
...
- #5849 `41259ca` contrib/init/coinbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Coinbit-Qt.app to Coinbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for coinbit-tx)
