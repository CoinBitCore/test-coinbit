Coinbit Core version 0.12.0 is now available from:

  <https://coinbit.org/bin/coinbit-core-0.12.0/>
...
backwards-compatible with pre-0.10 versions of Coinbit Core or other software:
...
pre-0.12 versions of Coinbit Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Coinbit Core version 0.11 or
earlier.
...
ECDSA signatures inside Coinbit transactions now use validation using
...
Previous versions of Coinbit Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Coinbit Core was to raise the default minimum
relay fee.

Coinbit Core 0.12 will have a strict maximum size on the mempool. The
...
Coinbit Core 0.12 also introduces new default policy limits on the length and
...
Coinbit Core 0.12 nodes. Coinbit Core will only allow replacement of
...
Note that the wallet in Coinbit Core 0.12 does not yet have support for
...
This allows running coinbitd without having to do any manual configuration.
...
Coinbit Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Coinbit Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Coinbit Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Coinbit Core 0.13, and it is at this time undecided whether the
...
Coinbit Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Coinbit Core automatically creates a hidden service to listen on, without
manual configuration. Coinbit Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Coinbit Core is listening, and
...
Coinbitd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Coinbit
Core to use floating fees. By default, Coinbit Core will use floating
fees.
...
(default: `0.0002` CTB/kB).

At all times, Coinbit Core will cap fees at `-maxtxfee=<x>` (default:
0.10) CTB.
Furthermore, Coinbit Core will never create transactions paying less than
...
`coinbit.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/CoinBitCore/coinbit/blob/v0.11.0/doc/release-notes.md#block-file-pruning).
...
Then, to tunnel a SSL connection on 29443 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 29443 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /coinbitrpc>
        ...
        # optional bypass coinbitd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< coinbitrpc:<password>
    </Location>
...
Restarting coinbitd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Coinbit-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
...
- #6600 `ef4945f` Include coinbit-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix coinbit-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of coinbit-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override COINBITD and COINBITCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [coinbit-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add COINBITD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libcoinbitconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "CTB" (Ross Nicoll)
