...
###   coinbit.org and coinbit.org and compares them.
...
WORKINGDIR="/tmp/coinbit_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://coinbit.org"
HOST2="https://coinbit.org"
BASEDIR="/bin/"
VERSIONPREFIX="coinbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'coinbit-',
...
if [ $? -ne 0 ]; then
   echo "coinbit.org failed to provide signature file, but coinbit.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "coinbit.org and coinbit.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Coinbit Core binary release signing key installed?"
   fi
