...
The RPM spec file provided here is for Coinbit-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://coinbit.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Coinbit git repository](https://github.com/CoinBitCore/coinbit) but are not part of
...
Currently only `Source10` is used, for the example `coinbit.conf` file.
...
in the coinbit git repository. At present only an SVG version of the Coinbit
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the coinbit
...
needed when 0.12.1 is released, a proper fix is already in the Coinbit git
master branch.
...
idea to build Coinbit Core against a static version of that library compiled
...
The `coinbit.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `coinbit-server` package. At this point, most RPM based
...
The `coinbit-server` package should have SELinux support. How to properly do
...
Coinbit-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/coinbit/). He has been
packaging Coinbit for Fedora at least since 2012.
...
4. I renamed the `coinbit` package that contains the Qt GUI to `coinbit-core` as
that appears to be how the general population refers to it, in contrast to
`coinbit-xt` or `coinbit-classic`. I wanted to make sure the general population
