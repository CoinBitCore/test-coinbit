#!/bin/bash
# Copyright (c) 2013 The Coinbit Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Coinbit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Coinbit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Coinbit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Coinbit 0.7 datadir."
    ;;
  3)
    echo "Detected Coinbit pre-0.8 datadir."
    ;;
  4)
    echo "Detected Coinbit 0.8 datadir."
    ;;
esac
